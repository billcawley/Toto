...
<table>
    ...
    <c:forEach items="${backups}" var="backup">
        <tr>
            <td>${backup.name}</td>
            <td>${backup.date}</td>
            <td><a href="/api/ManageDatabaseBackups?restoreBackup=${backup.name}&databaseId=${databaseId}" onclick="return clickRestore()" class="button small alt" title="Restore ${backup.name}"><span class="fa fa-eject" title="Restore"></span></a></td>
        </tr>
    </c:forEach>
    </tbody>
</table>
