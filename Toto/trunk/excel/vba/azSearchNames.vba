...
Private Sub azShowChildren(cParent As clsNode, keyname As Object)
   ...
   If keyname.Item("children").Count = 0 Then
      Exit Sub
   End If
   cParent.Expanded = False
   Dim azChild As Object
   For Each azChild In keyname.Item("children")
     Dim cChild As clsNode
     Set cChild = cParent.AddChild(azNameCount & "", azChild.Item("name"))
...
'        .AppName = Me.AppName
        ...
        azNameCount = 1
        For Each keyname In jsonNames.Item("names")
         ' add a Root node and make it bold
          Set cRoot = .AddRoot(sKey:=azNameCount & "", vCaption:=keyname.Item("name"))
          azNameCount = azNameCount + 1
          cRoot.Bold = True
...
End Sub
  Private Function findKey(Source As Object, keyGiven As Variant)
    Dim key As Variant
    Dim strKey As String
    For Each key In Source.Keys()
      If key = keyGiven Then
        strKey = key
        findKey = Source.Item(strKey)
        Exit Function
      End If
    Next
    findKey = ""
...
Public Sub mcEdit()
   ...
   If Not azName Is Nothing Then
      ...
      NameEdit.azNameValue.value = azName.Item("name")
      
      DataItems = findKey(azName, "mydataitems")
      NameEdit.DataItems = DataItems & ""
      
       Dim att As Variant
      For Each att In azName.Item("attributes").Keys()
          Dim AttName As String
          Dim AttValue As String
          AttName = att
          AttValue = azName.Item("attributes").Item(AttName)
...
Public Sub mcPaste()
   ...
   If origNode Is Nothing Then
    ...
    If Not origNode Is Nothing Then
       azReply = TellAzquo(origNode, cParent, pos)
       mcTree.Copy origNode, cParent, vBefore:=pos, bShowError:=True
    End If
    If bMove Then
     If Not origNode Is Nothing Then
         If Not origNode.ParentNode Is cParent And origNode.ParentNode.Level >= 0 Then
...
Private Function TellAzquo(cNode As clsNode, cParent As clsNode, newPosition As Integer)
            ...
            params = """operation"":""edit"",""id"":" & cNode.AzquoName.Item("id")
             If newPosition > 0 Then
               params = params & ",""newParent"":" & cParent.AzquoName.Item("id") & ",""newPosition"":" & newPosition
            Else
               params = params & ",""oldParent"":" & cParent.AzquoName.Item("id")
